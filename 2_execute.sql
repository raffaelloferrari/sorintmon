...
--  LOOP -- Decommentare nel caso lo sivoglia far eseguire ogni x minuti
    INSERT INTO sorint.sql_monitor (sql_id, key, sql_exec_start, sql_exec_id, status, first_refresh_time, last_refresh_time, sql_text, username,elapsed_time )
    SELECT v.sql_id, v.key, v.sql_exec_start, v.sql_exec_id, v.status, v.first_refresh_time, v.last_refresh_time, v.sql_text, v.username,v.elapsed_time/1000
      FROM v$sql_monitor v
       ...
       AND v.elapsed_time/1000 > REGEXP_SUBSTR(v.sql_text,'[^|]+',1,2)
       AND (v.status LIKE 'DONE%' OR (v.status = 'EXECUTING' AND (v.last_refresh_time - v.first_refresh_time) > 1/24/60 /* 1 min */))
       AND NOT EXISTS (SELECT NULL FROM sorint.sql_monitor t WHERE t.sql_id = v.sql_id AND t.key = v.key);

    FOR i IN (SELECT t.*, t.ROWID row_id FROM sorint.sql_monitor t WHERE t.capture_date IS NULL)
    LOOP
